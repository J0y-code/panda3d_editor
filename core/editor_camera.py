...
class EditorCamera(DirectObject):
    def __init__(self, base, viewport_area=(0.15, 0.0, 0.8, 1.0)):
        ...
        # États et paramètres
        self.enabled = True         # <--- ✅ nouveau flag
        self.is_on_script = False   # bloque quand un script prend la main
        self.move_speed = 25.0
        self.mouse_sensitivity = 0.25
        self.zoom_speed = 100.0

        # États clavier/souris
        self.keys = {"z": False, "s": False, "q": False, "d": False, "a": False, "e": False}
    ...
    def set_enabled(self, value: bool):
        """Active/désactive le contrôle de la caméra d’édition."""
        self.enabled = value
        print(f"[EditorCamera] {'Activée' if value else 'Désactivée'}")

    def toggle_enabled(self):
        """Bascule l’état enabled on/off."""
    ...
    def zoom_in(self):
        if not self.enabled or not self.is_mouse_in_viewport() or self.is_on_script:
            return
    ...
    def update(self, task):
        if not self.enabled:  # ignore les inputs quand désactivée
            return task.cont
