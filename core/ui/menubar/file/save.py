...
class Save:
    ...
    def save_scene_as(self, *args):
        ...
        # Field to type filename directly
        name_box = BoxLayout(size_hint_y=None, height=40)
        name_input = TextInput(hint_text='File name (optional)', multiline=False)
        ...
        popup = Popup(title="Save Scene", content=box, size_hint=(0.9, 0.9))
        popup.open()

        def do_save(*args):
            # récupère le dossier courant du chooser
            folder = Path(chooser.path)
            # Prioriser le champ de texte si renseigné, sinon la sélection du chooser
